...
const MainPage: React.FC<MainPageProps> = ({
      ...
      fileInput.onchange = async () => {
        ...
        if (!data.conversation || !data.messages) {
          NotificationService.handleError(
            "JSON must contain { conversation, messages }"
          );
          return;
        }
  ...
  const createButton = () => {
    ...
    button.className =
      "px-2 py-1 bg-gray-100 text-black dark:text-black dark:bg-gray-200 border border-gray-200 dark:border-gray-800 rounded-md shadow-md hover:bg-gray-200 dark:hover:bg-gray-100 focus:outline-hidden";
