...
const ChatSettingsForm: React.FC<ChatSettingsFormProps> = ({
  ...
  const [formData, setFormData] = useState<ChatSettings>(
    chatSettings || DUMMY_CHAT_SETTINGS
  );
  ...
  return (
    <div className="w-full md:max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto pt-3">
      <form
        ...
        className="bg-white dark:bg-gray-800 px-8 pt-6 pb-8 mb-4"
      >
        <AvatarFieldEditor
          readOnly={readOnly}
          image={
            formData?.icon ? formData.icon : { data: null, type: "raster" }
          }
        ...
        <div className="mb-4">
          <FormLabel
            readOnly={readOnly}
            label={`${t("name-header")}${readOnly ? "" : " *"}`}
            htmlFor="name"
            value={formData.name || t("non-applicable")}
          ></FormLabel>
          {readOnly ? (
        ...
        <div className="mb-4">
          <FormLabel
            readOnly={readOnly}
            label={t("description-header")}
            htmlFor="description"
            value={formData.description || t("non-applicable")}
          ></FormLabel>
          {readOnly ? (
            <p className="text-gray-700 dark:text-gray-300">
        ...
        <div className="mb-4">
          <FormLabel
            readOnly={readOnly}
            label={t("instructions-header")}
            htmlFor="instructions"
            value={formData.instructions || t("non-applicable")}
          ></FormLabel>
          {readOnly ? (
            <p className="text-gray-700 dark:text-gray-300">
        ...
        <div className="mb-4">
          <FormLabel
            ...
            label={t("seed-header")}
            htmlFor={"seed"}
            value={formData.seed || t("non-applicable")}
          ></FormLabel>
          {readOnly ? (
            <p className="text-gray-700 dark:text-gray-300">
