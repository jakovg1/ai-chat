...
export const UserProvider = ({ children }: UserProviderProps) => {
  const [userSettings, setUserSettings] = useState<UserSettings>(() => {
    ...
    const userTheme: UserTheme =
      storedUserTheme === "light" ||
      storedUserTheme === "dark" ||
      storedUserTheme === "system"
        ? storedUserTheme
        : defaultUserSettings.userTheme;

    const model =
      localStorage.getItem("defaultModel") || defaultUserSettings.model;
    const instructions =
      localStorage.getItem("defaultInstructions") ||
      defaultUserSettings.instructions;
    const speechModel =
      localStorage.getItem("defaultSpeechModel") ||
      defaultUserSettings.speechModel;
    const speechVoice =
      localStorage.getItem("defaultSpeechVoice") ||
      defaultUserSettings.speechVoice;

    const speechSpeedRaw = localStorage.getItem("defaultSpeechSpeed");
    const speechSpeed =
      speechSpeedRaw !== null
        ? Number(speechSpeedRaw)
        : defaultUserSettings.speechSpeed;
  ...
  useEffect(() => {
    if (
      userSettings.speechSpeed === null ||
      userSettings.speechSpeed === undefined ||
      userSettings.speechSpeed < 0.25 ||
      userSettings.speechSpeed > 4.0
    ) {
      localStorage.removeItem("defaultSpeechSpeed");
    } else {
      localStorage.setItem(
        "defaultSpeechSpeed",
        String(userSettings.speechSpeed)
      );
    }
